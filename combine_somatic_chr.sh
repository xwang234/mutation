...
do
  ...
  if [[ ! -f ${tumorname}_$chr.indel ]];then echo "${tumorname}_$chr.indel is missing!!!";fi
done
...
do
  ...
  then 
	...
	cat ${tumorname}_$chr.indel.Somatic.hc > ${tumorname}.indel.Somatic.hc
  else
    awk '{if (NR>1) print}' ${tumorname}_$chr.snp.Somatic.hc >> ${tumorname}.snp.Somatic.hc  #remove title
	awk '{if (NR>1) print}' ${tumorname}_$chr.indel.Somatic.hc >> ${tumorname}.indel.Somatic.hc
  fi
done
