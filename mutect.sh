...
mutect=/fh/fast/dai_j/CancerGenomics/Tools/Mutect/mutect-1.1.7.jar
...
qmin=${5:-"5"} #min base quality value filter set as 1 10 20
mapmin=${6:-"20"} #min mapping quality score filter
...
then
  ...
  #java $java_opts -jar $mutect --analysis_type MuTect --reference_sequence $reference --cosmic $cosmic --dbsnp $dbsnp --min_qscore 5 --required_maximum_alt_allele_mapping_quality_score 10 --input_file:normal $normalbam --input_file:tumor $tumorbam --out $output --vcf $outputvcf --coverage_file $outputwig --enable_extended_output #-nct ${SLURM_CPUS_ON_NODE}
  java $java_opts -jar $mutect --analysis_type MuTect --reference_sequence $reference --cosmic $cosmic --dbsnp $dbsnp --input_file:normal $normalbam --input_file:tumor $tumorbam --out $output --vcf $outputvcf --coverage_file $outputwig --enable_extended_output #-nct ${SLURM_CPUS_ON_NODE}
fi
