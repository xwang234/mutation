...
then
  ...
  outputdir=/fh/scratch/delete30/dai_j/henan/mutect4
  qmin=10 #for mutect1 qmin=1 for mutect3
  ...
  do
    ...
    tumorname=${wgstumors[$i]}
    echo sbatch /fh/fast/dai_j/CancerGenomics/Tools/wang/mutation/mutect.sh $normalbam $tumorbam $tumorname $outputdir $qmin
    sbatch /fh/fast/dai_j/CancerGenomics/Tools/wang/mutation/mutect.sh $normalbam $tumorbam $tumorname $outputdir $qmin
    sleep 1
  done
fi
